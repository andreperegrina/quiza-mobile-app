export const BACKGROUND = '#16161a';
export const PRIMARY_BUTTON = '#7f5af0';
export const PRIMARY_BUTTON_TEXT = '#fffffe';
export const SECONDARY_BUTTON = '#242629';
export const SECONDARY_BUTTON_TEXT = '#7f5af0';
export const TERTIARY_BUTTON = '#2cb67d';
export const TERTIARY_BUTTON_TEXT = '#fffffe';
export const QUATERNARY_BUTTON = '#72757e';
export const QUATERNARY_BUTTON_TEXT = '#fffffe';
export const HEADLINE = '#fffffe';
export const PARAGRAPH = '#94a1b2';
export const STROKE = '#010101';
export const SECONDARY = '#72757e';
export const MAIN = '#fffffe';
export const TERTIARY = '#2cb67d';
export const HIGHLIGHT = '#7f5af0';
export const CARD_BACKGROUND = '#242629';
